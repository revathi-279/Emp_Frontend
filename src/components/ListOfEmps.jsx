...
function ListOfEmps() {
  ...
  return (
    <div>
        <h1 className='text-3xl text-red-800 text-center pb-10'>List Of Employees</h1>
        <div className='grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-10'>
            {
                emps.map((empObj) => (
                    <div key={empObj._id} className='bg-red-100 p-5 rounded-3xl text-center'>
                    <p className='mb-4'>{empObj.email}</p>
                    <p className='mb-6'>{empObj.name}</p>
                        {/* buttons */}
                      <div className='flex justify-around'>
                    <button onClick = {()=>goToEmployee(empObj)} className='bg-red-200 text-red-900 p-3 rounded-2xl'>View</button>
                    <button onClick = {()=>goToEditEmployee(empObj)} className='bg-red-200 text-red-900 p-3 rounded-2xl'>Edit</button>
                     <button onClick = {()=>deleteEmpById(empObj._id)} className='bg-red-200 text-red-900 p-3 rounded-2xl'>Delete</button>
                      </div>
                    </div>
                ))
            }
